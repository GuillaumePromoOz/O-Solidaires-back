...
class RequestController extends AbstractController
{
    ...
    public function read(RequestRepository $requestRepository): Response
    {
        ...
        return $this->json($requests, 200, ['Access-Control-Allow-Origin' => '*'], ['groups' => 'requests_read']);
    }
    ...
    public function readItem(Request $request = null): Response
    {
        ...
        return $this->json($request, 200, ['Access-Control-Allow-Origin' => '*'], ['groups' => [
            'requests_read',
        ]]);
    }
    ...
    public function createUser(Request $request, EntityManagerInterface $entityManager, SerializerInterface $serializer, ValidatorInterface $validator)
    {
        ...
        if ($roleIndex !== "ROLE_BENEFICIARY"){
            return $this->json(['error' => 'Cet utilisateur n\'est pas un bénéficiaire'], Response::HTTP_NOT_FOUND, ['Access-Control-Allow-Origin' => '*']);
        }

        $entityManager->persist($userRequest);
        $entityManager->flush();
        return $this->json('Demande créée', Response::HTTP_CREATED, ['Access-Control-Allow-Origin' => '*']);
    }
    ...
    public function patch(RequestEntity $userRequest = null, EntityManagerInterface $em, SerializerInterface $serializer, ValidatorInterface $validator, Request $request)
    {
        ...
        if ($userRequest === null) {
            ...
            return $this->json(['error' => 'Demande non trouvée.'], Response::HTTP_NOT_FOUND, ['Access-Control-Allow-Origin' => '*']);
        }
        ...
        return $this->json(['message' => 'Demande modifiée.'], Response::HTTP_OK, ['Access-Control-Allow-Origin' => '*']);
    }
}
